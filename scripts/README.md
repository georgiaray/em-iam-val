...
### Quick Start (from ml-iam directory)

If ml-iam is in the standard location, you can run checks directly from the ml-iam root:

```bash
cd ../ml-iam
python ../em-iam-val/run_all.py --run_id xgb_04
```

Or with a custom ml-iam location:

```bash
export ML_IAM_ROOT=/custom/path/to/ml-iam
...
```bash
python run_all.py --run_id xgb_04 --report
```

Run an individual check:
```bash
python check_plausibility.py --run_id xgb_04
...
1. Create `<your_check>.py` with a `main()` function that uses
   `argparse` and accepts at minimum `--run_id`. Follow the same pattern as the existing checks: set `REPO_ROOT` using the `ML_IAM_ROOT` environment variable (with `../ml-iam` as default).
